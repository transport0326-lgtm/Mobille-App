...
import { saveActiveBooking, clearActiveBooking } from '../../utils/tokenStorage';
import { setCustomerSkipRestore } from '../../redux/slices/bookingSlice';
...
const BookingConfirmedScreen: React.FC<BookingConfirmedScreenProps> = ({ navigation, route }) => {
  ...
  const dotProgress = (() => {
    if (isAssignedPhase) {
      ...
      if (!initLoc) return 0;
      const totalToPickup = getDistance(initLoc.lat, initLoc.lng, pickupLat, pickupLng);
    ...
    if (!pickupLat || !pickupLng || !dropoffLat || !dropoffLng || !riderLat || !riderLng) return 0;
  ...
  const trackingMessage =
    status === 'assigned' ? 'Rider is on the way to pickup location' :
      status === 'arrived_at_pickup' ? 'Rider has picked up the parcel and is heading for delivery!' :
        status === 'in_transit' ? 'Parcel picked up! On the way to destination' :
          null;
  ...
  useEffect(() => {
    if (status === 'assigned' && riderLat && riderLng && !initialRiderLocRef.current) {
      initialRiderLocRef.current = { lat: riderLat, lng: riderLng };
    }
  }, [riderLat, riderLng, status]);

  // Reset animation to 0 instantly when phase switches from assigned → arrived_at_pickup
  // so dot doesn't slide backward
  ...
  // Persist active booking so the app can restore this screen on reopen
  useEffect(() => {
    if (routeBookingId) {
  ...
  // Hardware back button → dashboard (not the previous stack screen)
  useEffect(() => {
        ...
        <View style={styles.card}>
          <View style={styles.trackingHeader}>
            ...
            <View style={styles.trackingMeta}>
              {distanceKm != null && <Text style={styles.trackingMetaText}>{distanceKm.toFixed(1)} km</Text>}
              {etaMinutes != null && <View style={styles.etaBadge}><Text style={styles.etaBadgeText}>ETA {etaMinutes} min</Text></View>}
            </View>
          </View>
          ...
          <View style={styles.trackRow}>
            ...
            <View style={[styles.endDotPickup, isAssignedPhase && { backgroundColor: Colors.primary }]}>
            ...
            <View style={styles.lineContainer}>
              ...
              <Animated.View style={[styles.lineGreen, {
                width: progressAnim.interpolate({ inputRange: [0, 1], outputRange: ['0%', '100%'] }),
              }]} />
              <Animated.View style={[styles.riderDot, {
                left: progressAnim.interpolate({ inputRange: [0, 1], outputRange: ['0%', '93%'] }),
              }]}>
            ...
            {/* Right endpoint */}
            <View style={[styles.endDotDropoff, isAssignedPhase && { backgroundColor: '#22C55E' }]}>
          ...
          <View style={styles.trackLabels}>
            <Text style={styles.trackLabel} numberOfLines={1}>{trackLeftLabel}</Text>
            <Text style={[styles.trackLabel, { textAlign: 'right' }]} numberOfLines={1}>{trackRightLabel}</Text>
          </View>
        </View>
      ...
      <View style={styles.bottomBar}>
        ...
        {status === 'assigned' && (
          <TouchableOpacity
            ...
            activeOpacity={0.85}
            onPress={() => navigation.navigate('CancelBooking', { bookingId: routeBookingId ?? '' })}>
...
const styles = StyleSheet.create({
  ...
  cardSectionLabel: { fontSize: 13, fontWeight: '700', color: Colors.textGray, letterSpacing: 0.3 },
  ...
  trackingMessageText: { fontSize: 13, fontWeight: '600', color: Colors.secondary, textAlign: 'center' },
  ...
  etaBadge: { backgroundColor: '#E8F5E9', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 3 },
  etaBadgeText: { fontSize: 12, fontWeight: '700', color: '#16A34A' },

  trackRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 4, marginBottom: 10 },

  endDotPickup: { width: 16, height: 16, borderRadius: 8, backgroundColor: '#22C55E', alignItems: 'center', justifyContent: 'center', zIndex: 2 },
  endDotInner: { width: 7, height: 7, borderRadius: 4, backgroundColor: Colors.white },
  endDotDropoff: { width: 16, height: 16, borderRadius: 8, backgroundColor: '#E0E0E0', alignItems: 'center', justifyContent: 'center', zIndex: 2 },
  endDotInnerGray: { width: 7, height: 7, borderRadius: 4, backgroundColor: Colors.white },

  lineContainer: { flex: 1, height: 6, position: 'relative', justifyContent: 'center', marginHorizontal: 2 },
  lineGray: { position: 'absolute', left: 0, right: 0, height: 6, backgroundColor: '#E0E0E0', borderRadius: 3 },
  lineGreen: { position: 'absolute', left: 0, height: 6, backgroundColor: '#22C55E', borderRadius: 3 },
  riderDot: { position: 'absolute', width: 32, height: 32, borderRadius: 16, backgroundColor: Colors.white, alignItems: 'center', justifyContent: 'center', top: -13, elevation: 5, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4, borderWidth: 2, borderColor: '#22C55E' },
  riderDotIcon: { fontSize: 16 },
  riderDotOrange: { width: 12, height: 12, borderRadius: 6, backgroundColor: Colors.primary },

  trackLabels: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 2 },
  trackLabel: { fontSize: 11, color: Colors.textGray, fontWeight: '500', maxWidth: '45%' },
  ...
  chatBtn: {
    ...
    gap: 8, backgroundColor: Colors.secondary, borderRadius: 12, paddingVertical: 14,
  },
  ...
  cancelBtn: {
    ...
    gap: 8, borderWidth: 1.5, borderColor: Colors.primary, borderRadius: 12,
    paddingVertical: 13, backgroundColor: '#FFF5F5',
  },
  cancelBtnText: { color: Colors.primary, fontSize: 14, fontWeight: '700', flex: 0, textAlign: 'center' },
  ...
  riderAvatarSmallText: { fontSize: 11, fontWeight: '700', color: Colors.white },
});
